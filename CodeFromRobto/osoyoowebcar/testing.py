...
pi_ip_address='130.215.124.75'#'130.215.9.186' #'localhost'#'localhost'#'0.0.0.0'#'192.168.0.32' # replace 192.168.0.107 with your Raspberry Pi IP address
...
def changespeed(speed):
    #print("changeSpeed")
    pwm.set_pwm(ENB, 0, speed)#right
    pwm.set_pwm(ENA, 0, int(speed*.9))#left
...
def turnCam():#camDir):
    if(camDirX != 0 or camDirY != 0):
        print("turning cam, X: " + str(camDirX) + ", Y: " + str(camDirY))
    #return
...
def action(action,cmd):
    global carDir, camDirX, camDirY
    if action=='camera':
        print("action: camera, cmd: " + cmd)
        if cmd=='forward':
            camDirX = CamDirection.NONE.value
            ...
            camDirX = CamDirection.RIGHT.value
            camDirY = CamDirection.NONE.value
            print("camDirX: " + str(camDirX) + ", camDirY: " + str(camDirY))
        elif cmd=='back_right':
            camDirX = CamDirection.RIGHT.value
